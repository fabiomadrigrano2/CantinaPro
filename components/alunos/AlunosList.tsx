...
export default function AlunosList({ initialAlunos }: { initialAlunos: AlunoComConta[] }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (editingAluno) {
      ...
      await supabase.from("contas").update({ saldo: parseFloat(form.saldo_inicial) || 0, tipo: form.tipo_conta }).eq("aluno_id", editingAluno.id);
      ...
      // sync contas (best-effort)
      await supabase.from("contas").insert({ cantina_id: CANTINA_ID, aluno_id: aluno.id, saldo: parseFloat(form.saldo_inicial) || 0, tipo: form.tipo_conta });
